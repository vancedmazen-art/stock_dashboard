...
def draw_candle_chart(ticker, height=650, stop_loss=None, entry=None, entry_date=None, closed_trades_df=None):
    ...
    for _, row in ticker_divs.iterrows():
        ...
        if div_date in dates:
            ...
            ca_mark_points.append({
                "name": "DIV", "coord": [idx, float(df.iloc[idx]["low"]) * 0.96],
                "value": f"Div {amount:.3f}", "symbol": "circle", "symbolSize": 14,
    ...
    for _, row in ticker_splits.iterrows():
        ...
        ca_mark_points.append({
            "name": "SPLIT", "coord": [idx, float(df.iloc[idx]["high"]) * 1.05],
    ...
    if stop_loss is not None:
        mark_lines_data.append({"yAxis": float(stop_loss), "lineStyle": {"color": "#f87171", "width": 1.5, "type": "dashed"},
                                 "label": {"show": True, "formatter": f"SL {float(stop_loss):.3f}", "position": "insideEndTop", "color": "#f87171", "fontSize": 11}})
    ...
    function buildOption(startPct){{return{{backgroundColor:'#0f172a',animation:false,
      title:{{text:'EGX: '+TICKER,textStyle:{{color:'#d1fae5',fontSize:15,fontFamily:'DM Mono',fontWeight:'700'}},left:'1%',top:6}},
      tooltip:{{trigger:'item',axisPointer:{{type:'cross',crossStyle:{{color:'#4b6a57',width:1}},lineStyle:{{color:'#4b6a57',width:1,type:'dashed'}}}},
        backgroundColor:'#0f172a',borderColor:'#1e3a2a',borderWidth:1,padding:[8,12],
        formatter:function(p){{var v=p.value;
          if(!Array.isArray(v)||v.length<5){{
            if(p.seriesName==='EMA 20')return'<div style="font-family:DM Mono,monospace;font-size:12px"><span style="color:#facc15">EMA20</span> <b style="color:#e2e8f0">'+parseFloat(v).toFixed(3)+'</b></div>';
      ...
      xAxis:[
        {{type:'category',data:DATES,gridIndex:0,scale:true,boundaryGap:true,axisLine:{{lineStyle:{{color:'#1e3a2a'}}}},axisTick:{{show:false}},axisLabel:{{show:false}},splitLine:{{show:false}}}},
        {{type:'category',data:DATES,gridIndex:1,scale:true,boundaryGap:true,axisLine:{{lineStyle:{{color:'#1e3a2a'}}}},axisTick:{{show:false}},axisLabel:{{show:false}},splitLine:{{show:false}}}},
      ],
      yAxis:[
        {{scale:true,gridIndex:0,position:'right',splitLine:{{show:false}},axisLine:{{show:false}},axisTick:{{show:false}},
          axisLabel:{{color:'#d1fae5',fontSize:13,fontWeight:'bold',fontFamily:'DM Mono',margin:8,formatter:function(v){{return parseFloat(v).toFixed(3);}}}}}},
        {{scale:true,gridIndex:1,position:'right',splitLine:{{show:false}},axisLine:{{show:false}},axisTick:{{show:false}},
          axisLabel:{{color:'#4b6a57',fontSize:11,fontFamily:'DM Mono',formatter:function(v){{return fmtVol(v);}}}},name:'Vol',nameTextStyle:{{color:'#4b6a57',fontSize:10}}}},
      ],
      dataZoom:[
        {{type:'inside',xAxisIndex:[0,1],start:startPct,end:100,zoomOnMouseWheel:true,moveOnMouseWheel:false,preventDefaultMouseMove:false}},
        {{type:'slider',xAxisIndex:[0,1],start:startPct,end:100,bottom:4,height:18,borderColor:'#1e3a2a',backgroundColor:'#0a1a12',
          ...
          fillerColor:'rgba(16,185,129,0.08)',handleStyle:{{color:'#10b981'}},textStyle:{{color:'#4b6a57',fontSize:9}}}},
      ],
      series:[
        {{name:TICKER,type:'candlestick',xAxisIndex:0,yAxisIndex:0,data:CANDLES,
          itemStyle:{{color:'transparent',color0:'transparent',borderColor:'#10b981',borderColor0:'#f87171',borderWidth:1}},
          markPoint:{{data:MARK_PTS,animation:false}},
          markLine:{{symbol:['none','none'],animation:false,silent:true,data:MARK_LINES}}}},
        {{name:'EMA 20',type:'line',xAxisIndex:0,yAxisIndex:0,data:EMA,smooth:false,lineStyle:{{color:'#facc15',width:1.5}},symbol:'none',z:3}},
    ...
    document.getElementById('chart').addEventListener('wheel',function(e){{
      ...
      var opt=chart.getOption(),dz=opt.dataZoom[0],span=dz.end-dz.start,step=span*0.08*(e.deltaY>0?1:-1);
    ...
    function setMode(mode){{
      drawMode=mode;isDragging=false;dragStart=null;previewActive=false;
      document.getElementById('trend-hint').style.display=mode==='trend'?'inline':'none';
      ['hline','vline','trend','none'].forEach(function(id){{var btn=document.getElementById('btn-'+id);if(btn)btn.classList.toggle('active',(mode===null&&id==='none')||id===mode);}});
      chart.setOption({{dataZoom:[{{type:'inside',disabled:!!mode}}]}});
      chart.getZr().setCursorStyle(mode?'crosshair':'default');
      if(!mode)clearPreview();
    }}
    function pixelToData(pixelX,pixelY){{var dp;try{{dp=chart.convertFromPixel({{gridIndex:0}},[pixelX,pixelY]);}}catch(e){{return null;}}if(!dp||dp.length<2)return null;var idx=Math.round(dp[0]);if(idx<0||idx>=DATES.length)return null;return{{idx:idx,date:DATES[idx],price:dp[1]}};}}
    function clearPreview(){{previewActive=false;chart.setOption({{series:[{{id:'__preview__',type:'scatter',xAxisIndex:0,yAxisIndex:0,data:[],symbol:'none',markLine:{{symbol:['none','none'],silent:true,animation:false,data:[]}}}}]}},{{replaceMerge:[]}});}}
    function drawPreview(x1idx,y1,x2idx,y2){{if(x1idx===x2idx)return;var slope=(y2-y1)/(x2idx-x1idx),x0=0,xEnd=DATES.length-1;previewActive=true;chart.setOption({{series:[{{id:'__preview__',type:'scatter',xAxisIndex:0,yAxisIndex:0,data:[],symbol:'none',markLine:{{symbol:['none','none'],silent:true,animation:false,lineStyle:{{color:'#facc1588',width:1.5,type:'dashed'}},data:[[{{xAxis:DATES[x0],yAxis:y1+slope*(x0-x1idx)}},{{xAxis:DATES[xEnd],yAxis:y1+slope*(xEnd-x1idx)}}]]}}}}]}},{{replaceMerge:[]}});}}
    var zr=chart.getZr(),canvas=document.querySelector('#chart canvas');
    zr.on('mousedown',function(e){{if(!drawMode)return;var d=pixelToData(e.offsetX,e.offsetY);if(!d)return;
    ...
    window.addEventListener('mouseup',function(e){{
      ...
      var rect=canvas?canvas.getBoundingClientRect():null,ox=rect?e.clientX-rect.left:e.offsetX,oy=rect?e.clientY-rect.top:e.offsetY;
    ...
    function clearAll(){{drawnLines=[];isDragging=false;dragStart=null;clearPreview();renderDrawn();}}
    function renderDrawn(){{var ml=[];drawnLines.forEach(function(ln){{
      if(ln.type==='hline'){{ml.push({{yAxis:ln.price,lineStyle:{{color:ln.color,width:1.5,type:'solid'}},label:{{show:true,formatter:ln.price.toFixed(3),position:'insideEndTop',color:ln.color,fontSize:10,fontFamily:'DM Mono'}}}});}}
      else if(ln.type==='vline'){{ml.push([{{xAxis:ln.date,yAxis:'min',lineStyle:{{color:ln.color,width:1.5,type:'solid'}},label:{{show:true,formatter:ln.date,position:'insideEndTop',color:ln.color,fontSize:10,fontFamily:'DM Mono'}}}},{{xAxis:ln.date,yAxis:'max'}}]);}}
      else if(ln.type==='trend'){{var slope=(ln.y2-ln.y1)/(ln.x2-ln.x1),x0=0,xEnd=DATES.length-1;ml.push([{{xAxis:DATES[x0],yAxis:ln.y1+slope*(x0-ln.x1),lineStyle:{{color:ln.color,width:1.5,type:'solid'}},label:{{show:false}}}},{{xAxis:DATES[xEnd],yAxis:ln.y1+slope*(xEnd-ln.x1)}}]);}}
    }});chart.setOption({{series:[{{id:'__drawn__',type:'scatter',xAxisIndex:0,yAxisIndex:0,data:[],symbol:'none',markLine:{{symbol:['none','none'],silent:true,animation:false,data:ml}}}}]}},{{replaceMerge:[]}});}}
    renderDrawn();
    </script></body></html>
    """)
...
def fetch_latest_news(symbol, max_items=3):
    ...
    def _parse_items(items):
        ...
        for news in items:
            news_id = news.get("id", "")
            if not news_id or news_id in seen_ids: continue
            title = news.get("title", "")
            syms = [s.get("symbol","").replace("EGX:","") for s in news.get("relatedSymbols",[]) if "EGX:" in s.get("symbol","")]
            if not (symbol_upper in [s.upper() for s in syms] or symbol_upper in title.upper()): continue
            seen_ids.add(news_id)
            found.append({"title": title, "url": f"https://www.tradingview.com{news.get('storyPath','')}", "provider": news.get("provider",{}).get("name",""), "date": _fmt_date(news.get("published",0))})
        return found

    try:
        r = requests.get(f"https://news-mediator.tradingview.com/news-flow/v2/news?filter=lang%3Aen&filter=symbol%3A{egx_symbol}&client=screener", timeout=10)
        r.raise_for_status()
        result.extend(_parse_items(r.json().get("items",[])))
    except: pass
    if len(result) < max_items:
        try:
            r = requests.get("https://news-mediator.tradingview.com/news-flow/v2/news?filter=lang%3Aen&filter=market%3Astock&filter=market_country%3AEG&client=screener", timeout=10)
            r.raise_for_status()
            result.extend(_parse_items(r.json().get("items",[])))
        except: pass
...
def metric_card(label, value, color="#d1fae5"):
    ...
    <div style="background:#0f172a;border:1px solid #1e3a2a;border-radius:9px;
      ...
      <div style="font-size:10px;color:#4b6a57;text-transform:uppercase;
                  letter-spacing:0.1em;font-family:'DM Mono',monospace;margin-bottom:3px;">{label}</div>
...
def pnl_color(v):
    try:
        return "#34d399" if float(v) >= 0 else "#f87171"
    except:
        return "#d1fae5"
...
with c1:
    st.markdown("# 📡 EGX Pattern Scanner")
with c2:
...
with st.sidebar:
    st.markdown("### 📊 Summary")
    st.metric("Open Trades",   len(df_open))
    st.metric("Closed Trades", len(df_closed))
    if len(df_closed) > 0:
        wins = (df_closed['PnL_%'] > 0).sum()
        st.metric("Win Rate", f"{wins/len(df_closed)*100:.1f}%")
        st.metric("Avg PnL",  f"{df_closed['PnL_%'].mean():.2f}%")
        st.metric("Median PnL", f"{df_closed['PnL_%'].median():.2f}%")

    st.markdown("---")
    if len(df_open) > 0:
        pos = df_open[df_open['PnL_%'] > 0]
        neg = df_open[df_open['PnL_%'] < 0]
        if len(pos) > 0:
    ...
    <div style="font-size:10px;color:#4b6a57;margin-bottom:6px">CONTACT</div>
...
# TABS
# ---------------------------
tab_open, tab_closed, tab_summary, tab_charts, tab_egx30 = st.tabs([
    f"📂 Open Trades ({len(df_open)})",
    f"📋 Closed Trades ({len(df_closed)})",
    "📊 Summary",
    "📈 Charts",
    "📊 EGX30",
])
...
def trade_panel(source_df, session_key, is_open=True):
    if source_df.empty:
    ...
    # get all rows for this ticker, show most recent
    ...
    with col_chart:
        ...
        ticker_closed = df_closed[df_closed['Ticker'] == selected].copy()
        draw_candle_chart(selected, height=650,
                          stop_loss=sl, entry=en, entry_date=ed,
                          closed_trades_df=ticker_closed if len(ticker_closed) > 0 else None)

        # trade history expander
        if len(rows) > 1 or not is_open:
            hist = ticker_closed if not is_open else rows
            with st.expander(f"📋 Trade History — {selected} ({len(hist)} trades)", expanded=False):
                show_cols = [c for c in ['Entry_Date','Exit_Date','Entry_Price','Exit_Price',
                                         'PnL_%','Bars_Held','Exit_Reason','Max_Gain','Max_Loss'] if c in hist.columns]
                st.dataframe(fix_df(hist[show_cols].sort_values('Entry_Date', ascending=False)),
...
with tab_open:
    ...
    if len(df_open) > 0:
        ...
        c1.metric("Total",    len(df_open))
        c2.metric("Positive", (df_open['PnL_%'] > 0).sum())
        c3.metric("Avg PnL",  f"{df_open['PnL_%'].mean():.2f}%")
        c4.metric("Best",     f"{df_open['PnL_%'].max():.2f}%")
    trade_panel(df_open, "open_ticker", is_open=True)
...
with tab_closed:
    ...
    if len(df_closed) > 0:
        ...
        c2.metric("Win Rate", f"{wins/len(df_closed)*100:.1f}%")
        c3.metric("Avg PnL",  f"{df_closed['PnL_%'].mean():.2f}%")
        c4.metric("Median",   f"{df_closed['PnL_%'].median():.2f}%")
    ...
    trade_panel(df_closed, "closed_ticker", is_open=False)

with tab_summary:
    st.markdown("### 📊 Ticker Summary")
    if not df_summary.empty:
        # sort by Median_PnL
        df_summary_sorted = df_summary.sort_values('Median_PnL', ascending=False)

        # highlight best tickers
        c1, c2, c3 = st.columns(3)
        if len(df_summary_sorted) > 0:
            best = df_summary_sorted.iloc[0]
            c1.metric("🏆 Best Ticker",   best['Ticker'])
            c2.metric("Win Rate",         f"{best['Win_Rate']:.1f}%")
            c3.metric("Median PnL",       f"{best['Median_PnL']:.2f}%")

        st.dataframe(fix_df(df_summary_sorted), use_container_width=True, height=600)
...
with tab_charts:
    ...
    if chart_symbol:
        ...
        if len(sym_open) > 0:
            ...
            ed2 = r.get('Entry_Date', None)

        draw_candle_chart(chart_symbol, height=700,
                          stop_loss=sl2, entry=en2, entry_date=ed2,
        ...
        if len(sym_closed) > 0:
            with st.expander(f"📋 Trade History — {chart_symbol} ({len(sym_closed)} trades)", expanded=False):
                show_cols = [c for c in ['Entry_Date','Exit_Date','Entry_Price','Exit_Price',
                                         'PnL_%','Bars_Held','Exit_Reason','Max_Gain','Max_Loss'] if c in sym_closed.columns]
...
with tab_egx30:
    ...
    draw_candle_chart('EGX30', height=700,
                      closed_trades_df=df_closed[df_closed['Ticker']=='EGX30'] if len(df_closed[df_closed['Ticker']=='EGX30']) > 0 else None)
