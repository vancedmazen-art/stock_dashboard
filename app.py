...
with tab1:
    ...
    st.dataframe(fix_pyarrow_df(new_buys_with_strategy[['Ticker', 'Entry_Date', 'Entry_Price', 'Entry_Volume','Breaks_Trendline','Testing_Anchor','Clears_Anchor','Stop_Loss','Target_Price','Risk_%','Reward_%', 'RR_Ratio']]), use_container_width=True, height=300)
    ...
    st.dataframe(fix_pyarrow_df(holds_with_strategy[['Ticker',  'Current_Price','Entry_Price', 'Entry_Date','Trade_PnL_%', 'Days_Held','Breaks_Trendline', 'Target_Price', 'Reward_%','Target_Hit','Current_Clears_Anchor','Trendline_Hit','RR_Ratio']]), use_container_width=True, height=200)
...
with tab2:
    ...
    with left_col:
        ...
        if len(current_stock_df) > 0:
            ...
            st.dataframe(fix_pyarrow_df(current_stock_df[['Entry_Date', 'Entry_Price','Current_Price', 'Trade_PnL_%', 'Days_Held']]), use_container_width=True, height=200)
        else:
...
with tab4:
    ...
    if Testing_Anchor:  # Only if selections made
        filtered_history = filtered_history[filtered_history['Testing_Anchor'] == False]
